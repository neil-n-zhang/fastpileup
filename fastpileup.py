...
def pileup_region(args):
    ...
    for start,end in blocks:        
        ...
        out = '{sa}_{c}_{s}_{e}.txt'.format(sa=args.sample,c=chr,s=start,e=end)
        f = pool.apply_async(worker, [region,out,args.bam,args.ref,args.parameters])
        outfiles.append(out)
    pool.close()
    pool.join()

    cmd = 'cat {i} > {o}'.format(i=' '.join(outfiles), o=args.sample+"_pileup.txt")
    subprocess.check_output(cmd, shell=True)
    for f in outfiles:
        os.remove(f)
...
def pileup_bed(args):
    ...
    for sub_bed in sub_bed_files:        
        print (sub_bed)
        out = '{sa}_{s}_out.txt'.format(sa=args.sample,s=sub_bed.replace(".bed",""))
...
if __name__ == '__main__':
    ...
    # python3 pileup.py region -sample test -bam test1.bam -reg 1:11931-12090 -threads 4 -parameters "-a -B -q0 -Q10" -ref /hs37d5.fa
    # python3 pileup.py bed -sample test -bam test1.bam -bed test.bed -threads 4 -parameters "-a -B -q0 -Q10" -ref /hs37d5.fa